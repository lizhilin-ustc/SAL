...
def parse_args():
    ...
    parser.add_argument('--q_val', default=0.5, type=float)  # 0.5

    # inference parameters
    parser.add_argument('--inference_only', action='store_true', default=False)
    parser.add_argument('--class_th', type=float, default=0.25)     # 0.25
    ...
    parser.add_argument('--nms_alpha', default=0.3, type=float)    # 0.35
    parser.add_argument('--nms_thresh', default=0.4, type=float)    # 0.4
    parser.add_argument('--load_weight', default=False, action='store_true')
    
    # system parameters
    parser.add_argument('--num_workers', type=int, default=8) # 8
    parser.add_argument('--seed', type=int, default=1, help='random seed (-1 for no manual seed)')  # 1
...
class Config(object):
    def __init__(self, args):
        ...
        self.act_thresh = np.arange(0.1, 1.0, 0.1)      #### # np.arange(0.1, 1.0, 0.1)
        self.act_thresh1 = np.arange(0.1, 0.95, 0.1)     #### # np.arange(0.1, 1.0, 0.1)
        ...
        self.num_segments_list = [100, 300, 750, 1000]
