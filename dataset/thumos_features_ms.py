...
class ThumosFeature(Dataset):
    def __init__(self, data_path, mode, modal, feature_fps, num_segments_list, len_feature, sampling, seed=-1, supervision='weak'):
        if seed >= 0:
    ...
    def get_data(self, index):
        ...
        for i in range(len(self.num_segments_list)):
            if self.modal == 'all':
                ...
                if self.sampling == 'random':
                    sample_idx = self.random_perturb(rgb_feature.shape[0], self.num_segments_list[i])
                elif self.sampling == 'uniform':
                    sample_idx = self.uniform_sampling(rgb_feature.shape[0], self.num_segments_list[i])
                else:
                ...
                if self.sampling == 'random':
                    sample_idx = self.random_perturb(feature.shape[0], self.num_segments_list[i])
                elif self.sampling == 'uniform':
                    sample_idx = self.uniform_sampling(feature.shape[0], self.num_segments_list[i])
                else:
                    raise AssertionError('Not supported sampling !')
            ...
            # feature_mc = torch.stack(feature_list,dim=0)
        return feature_list, vid_num_seg, sample_idx
    ...
    def random_perturb(self, length, num_segments):
    ...
    def uniform_sampling(self, length, num_segments):
        if num_segments == length:
            return np.arange(num_segments).astype(int)
        samples = np.arange(num_segments) * length / num_segments
